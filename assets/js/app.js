...
$(window).on('load', function() {
    ...
    $('section[id]').each((i, el) => {
        sections.push( {
            id: '#'+ $(el).attr('id'),
            el: $(el),
            scrollPosition:  $(el).offset().top,
            height: $(el).height(),
            range: $(el).offset().top + $(el).height()
        })
    });

    setScrollPosition(sections);
...
function setScrollPosition(sections) {
         // Save the actual section
         let position = $(window).scrollTop() + $('.header').height() + 20;
         let currentSection = sections.find(x => position >= x.scrollPosition && position <= x.range);
         if (currentSection) {
             localStorage.setItem('sectionId', currentSection.id)
         }
}
